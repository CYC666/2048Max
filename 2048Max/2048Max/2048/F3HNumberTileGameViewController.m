...
@property (nonatomic, strong) F3HScoreView *BestRcoreView;
...
- (void)setupGame {
    ...
    if (self.useScoreView) {
        ...
        bestRcoreView = [F3HScoreView scoreViewWithCornerRadius:6
                                                backgroundColor:[UIColor darkGrayColor]
                                                      textColor:[UIColor whiteColor]
                                                       textFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:16]
                                                         isBest:YES];
        self.BestRcoreView = bestRcoreView;
    }
    ...
    CGFloat cellWidth = floorf((kScreenWidth - padding*(self.dimension+1))/((float)self.dimension));
    if (cellWidth < 30) {
        cellWidth = 30;
    }
    F3HGameboardView *gameboard = [F3HGameboardView gameboardWithDimension:self.dimension
                                                               ...
                                                               cellPadding:padding
                                                              cornerRadius:6
                                                           backgroundColor:Color_FromRGB(0xb5a99d)
                                                           foregroundColor:Color_FromRGB(0xcbc1b7)];
    ...
    if (self.useScoreView) {
        ...
        CGRect bestRcoreFrame = scoreView.frame;
        bestRcoreFrame.origin.x = 0.5*(self.view.bounds.size.width - scoreFrame.size.width);
        bestRcoreFrame.origin.y = currentTop - scoreView.frame.size.height - 10;
        bestRcoreView.frame = bestRcoreFrame;
        [self.view addSubview:bestRcoreView];
...
- (void)viewDidLoad {
    ...
    closeButton.center = CGPointMake(kScreenWidth * 0.5, kScreenHeight - 80);
...
- (void)followUp {
    ...
    if ([self.model userHasWon]) {
        ...
        if ([self.model userHasLost]) {
            ...
            // 弹框提示是否执行
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Defeat!"
                                                                           message:@"You lost..."
                                                                    preferredStyle:UIAlertControllerStyleAlert];
...
- (void)scoreChanged:(NSInteger)newScore {
    ...
    NSString *key = [NSString stringWithFormat:@"BEST%ld", self.dimension];
    NSInteger bestScore = [user integerForKey:key];
    
    if (newScore >= bestScore) {
        // 设置最佳纪录
        self.BestRcoreView.score = newScore;
        [user setInteger:newScore forKey:key];
    } else {
        // 设置最佳纪录
        self.BestRcoreView.score = bestScore;
    }
