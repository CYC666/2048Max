...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.headerImageView = [[UIImageView alloc]initWithFrame:CGRectMake(5, 5, self.frame.size.width - 10, self.frame.size.width - 10)];
...
-(void)tapGesture:(UITapGestureRecognizer *)sender {
    ...
    if ([_delegate respondsToSelector:@selector(tapAction:)]) {
        [_delegate tapAction:self.infoDict];
    }
}

-(void)layoutSubviews {
    self.nameLabel.text = [NSString stringWithFormat:@"游戏规格 %@x%@",self.infoDict[@"number"], self.infoDict[@"number"]];
