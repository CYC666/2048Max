...
-(void)requestSourceData:(BOOL)needLoad{
    ...
    NSArray *images = @[@"2",@"3",@"4",@"5",@"6",@"7",@"8",@"9",@"10"];
    for (int i = 0; i<images.count; i++) {
        [objectArray addObject:@{@"number":images[i],
                                 @"image":[NSString stringWithFormat:@"%@",images[i]]}];
    }
...
- (void)tapAction:(NSDictionary *)infoDic {
